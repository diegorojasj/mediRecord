...
function matchesFilters(p: Patient, search: string, adv: FilterValues): boolean {
  ...
  if (adv.occupation && !p.occupation?.toLowerCase().includes(adv.occupation.toLowerCase()))
    return false;
  ...
  if (
    adv.stateProvince &&
    !p.address.state_province.toLowerCase().includes(adv.stateProvince.toLowerCase())
  )
    return false;
  if (adv.country && !p.address.country.toLowerCase().includes(adv.country.toLowerCase()))
    return false;
  ...
  if (
    adv.affiliationNumber &&
    !p.health_insurance?.affiliation_number?.includes(adv.affiliationNumber)
  )
    return false;
  if (
    adv.insuranceProvider &&
    !p.health_insurance?.provider?.toLowerCase().includes(adv.insuranceProvider.toLowerCase())
  )
    return false;
  return true;
}
        ...
        <ExpandableCardList
          ...
          renderRow={(p) => (
            <div className="flex items-center gap-3 w-full pr-2">
              <span
                className={`inline-flex size-9 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${avatarColor(fullName(p))}`}
              >
                {initials(p)}
              </span>
            ...
            <div className="relative">
              ...
              <div className="flex gap-4 flex-wrap">
                ...
                <div className="min-w-[120px] flex-1">
                  ...
                  <p className="mb-0.5 mt-2 text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">
                    Insurance
                  </p>
                  {p.health_insurance ? (
                    <div className="flex flex-col gap-px">
